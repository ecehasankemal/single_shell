...
// close_all_files fonksiyonu tanımlanır
void close_all_files()
{
    // out_fd ve err_fd dosya tanımlayıcıları kapatılır
    close(out_fd);
    close(err_fd);
}
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (strncmp(inbuf, "exit", 4) == 0)
        {
            ...
            write_output(timebuf);
            // Açık olan tüm dosyaları kapatmak için close_all_files() fonksiyonu kullanılır
            close_all_files();
